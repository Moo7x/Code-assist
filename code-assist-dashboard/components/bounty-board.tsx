...
type BountyStatus = "waiting" | "sleeping" | "urgent"
...
const bounties: Bounty[] = [
  {
    id: "#1024",
    errorSignature: "Cargo Build Failed",
    language: "Rust",
    context: ["Polkadot SDK", "Linux", "x86_64"],
    reward: "5.00",
    status: "waiting",
    statusLabel: "Agent Waiting",
    codeSnippet: `fn main() {\n    let data: Vec<u32> = vec![1, 2, 3];\n    // ERROR: cannot borrow as mutable\n    let result = data.iter().map(|x| x * 2);\n    println!("{:?}", result);\n}`,
  },
  {
    id: "#1023",
    errorSignature: "Import Error: Module Not Found",
    language: "Python",
    context: ["Docker", "v3.11"],
    reward: "3.50",
    status: "sleeping",
  ...
  {
    id: "#1022",
    errorSignature: "TypeError: Cannot read properties",
    language: "TypeScript",
    context: ["Node 20", "Next.js 14"],
    reward: "4.00",
    status: "waiting",
    statusLabel: "Agent Waiting",
    codeSnippet: `export async function getData() {\n  const res = await fetch("/api/users");\n  const data = await res.json();\n  // TypeError: Cannot read properties of undefined\n  return data.users.map((u: any) => u.name);\n}`,
  },
  {
    id: "#1021",
    errorSignature: "Gas Limit Exceeded",
    language: "Solidity",
    context: ["EVM", "v0.8.19"],
    reward: "8.00",
    status: "urgent",
    statusLabel: "Agent Urgent",
    codeSnippet: `function batchTransfer(\n    address[] memory recipients,\n    uint256[] memory amounts\n) external {\n    // Gas limit exceeded for large arrays\n    for (uint i = 0; i < recipients.length; i++) {\n        token.transfer(recipients[i], amounts[i]);\n    }\n}`,
  },
  {
    id: "#1020",
    errorSignature: "Segmentation Fault",
    language: "C++",
    context: ["GCC 13", "Linux"],
    reward: "6.00",
    status: "sleeping",
    statusLabel: "Agent Sleeping",
    codeSnippet: `#include <vector>\nint main() {\n    std::vector<int> nums;\n    // Segfault: accessing out of bounds\n    for (int i = 0; i <= nums.size(); i++) {\n        nums[i] = i * 2;\n    }\n    return 0;\n}`,
  },
  {
    id: "#1019",
    errorSignature: "Panic: index out of range",
    language: "Go",
    context: ["Docker", "v1.21"],
    reward: "4.50",
    status: "waiting",
    statusLabel: "Agent Waiting",
...
const statusConfig: Record<BountyStatus, { color: string; dot: string; bg: string }> = {
